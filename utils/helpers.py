...
def get_year_bounds(year_mode: str, selected_year: int, include_previous: bool = True) -> DateBounds:
    ...
    Parameters:
        year_mode (str): 'ytd' for calendar year, or 'fytd' for fiscal year (Jul 1 → Jun 30).
    ...
    if year_mode == 'fytd':
        ...
        raise ValueError('year_mode must be either "fytd" or "ytd"')
...
def add_quarter(df: pd.DataFrame, date_col: str, year_mode: str = "ytd") -> pd.DataFrame:
    """
    ...
    Parameters:
        ...
        year_mode (str): 'ytd' for calendar year (default) or 'fytd' for fiscal year (starting July 1)
